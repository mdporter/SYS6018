...
ggplot(data.frame(x,y), aes(x,y)) + 
  geom_point() + 
  geom_line(data=data.frame(x=xseq, y=yhat1), col="black") + 
...
#- ggplot2 plot
poly.data = data.frame(x = xseq, linear=yhat1, quadratic=yhat2) %>% 
  gather(model, y, -x)
ggplot(data.frame(x,y), aes(x,y)) + 
  geom_point() + 
...
ggplot(data.frame(x,y), aes(x,y)) + 
  geom_point() + 
  geom_line(data=data.frame(x=xseq, y=knn.20$pred)) + 
  geom_vline(xintercept=c(0, .4, .62), col="lightblue")  + 
  scale_x_continuous(breaks=seq(0, 1, by=.20)) + 
  ggtitle("kNN k=20")
...
poly_eval <- function(deg, data.train, data.test){
  if(deg==0) m = lm(y~1, data=data.train)  # intercept only model
  else m = lm(y~poly(x, degree=deg), data=data.train) # polynomial
  p = length(coef(m))                      # number of parameters
  mse.train = mean(m$residuals^2)          # training MSE
  yhat = predict(m, newdata=data.test)     # predictions at test X's
  mse.test = mean( (data.test$y - yhat)^2 )# test MSE
  data.frame(degree=deg, edf=p, mse.train, mse.test)
}

#-- Evaluate polynomials
Deg = c(1:6, seq(8, 20, by=2))   # sequence of Degrees
poly.eval = data.frame()
for(deg in Deg){
  tmp = poly_eval(deg, data.train=data.frame(x,y), data.test=data.frame(x=xtest, y=ytest))
...
knn_eval <- function(k, data.train, data.test){
  knn = knn.reg(data.train[,'x', drop=FALSE], 
                y = data.train$y, 
                test=data.train[,'x', drop=FALSE], 
                k=k)
  edf = nrow(data.train)/k        # effective dof (edof)
  r = data.train$y-knn$pred        # residuals on training data  
  mse.train = mean(r^2)            # training MSE
  knn.test = knn.reg(data.train[,'x', drop=FALSE], 
                     y = data.train$y, 
                     test=data.test[,'x', drop=FALSE], 
                     k=k)
  r.test = data.test$y-knn.test$pred # residuals on test data
  mse.test = mean(r.test^2)          # test MSE
  data.frame(k=k, edof=edf, mse.train, mse.test)
}
...
knn.eval = data.frame()
for(k in K){
  tmp = knn_eval(k, data.train=data.frame(x,y), data.test=data.frame(x=xtest, y=ytest))
